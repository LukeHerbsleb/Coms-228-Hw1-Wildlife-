...
public abstract class Living {
	...
	protected void census(int population[]) {
		...
		if (row > 0 && column > 0) { // top left is inside plain
			population[plain.grid[row - 1][column - 1].who().ordinal()] += 1; // adds one to the correct population spot
		} // else do nothing
		if (row > 0) { // top middle is inside
			population[plain.grid[row - 1][column].who().ordinal()] += 1;
		}
		if (row > 0 && column < plain.getWidth() - 1) { // top right
			population[plain.grid[row - 1][column + 1].who().ordinal()] += 1;
		}
		if (column > 0) { // middle left
			population[plain.grid[row][column - 1].who().ordinal()] += 1;
		}
		// skip the middle
		if (column < plain.getWidth() - 1) { // middle right
			population[plain.grid[row][column + 1].who().ordinal()] += 1;
		}
		if (row < plain.getWidth() - 1 && column > 0) { // bottom left
			population[plain.grid[row + 1][column - 1].who().ordinal()] += 1;
		}
		if (row < plain.getWidth() - 1) { // bottom middle
			population[plain.grid[row + 1][column].who().ordinal()] += 1;
		}
		if (row < plain.getWidth() - 1 && column < plain.getWidth() - 1) { // bottom right
			population[plain.grid[row + 1][column + 1].who().ordinal()] += 1;
		}
	}
